...
def parse_args():
    ...
    p.add_argument("--resume",      default=None,
                   help="Path to checkpoint to resume from  "
                        "(e.g. checkpoints/best_random.pt).  "
                        "Restores model weights, optimizer, scheduler, and epoch counter.")
    p.add_argument("--early_stop",  type=int, default=10,
                   help="Stop training if val R² does not improve for this many epochs "
...
def main():
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        #   new format: "model_state_dict"  (saved by this script)
        #   old format: "model"             (saved by the Colab version)
        state_dict = ckpt.get("model_state_dict") or ckpt.get("model")
        if state_dict is None:
            raise KeyError("Checkpoint has neither 'model_state_dict' nor 'model' key.")
        ...
        if "optimizer_state_dict" in ckpt:
            ...
            log.warning("  ⚠ No optimizer state in checkpoint — Adam momentum reset. "
                        "LR warmup may be needed for the first few epochs.")

        # Restore scheduler (if saved); otherwise fast-forward to correct LR
        if "scheduler_state_dict" in ckpt:
            ...
            for _ in range(completed):
                sched.step()
            log.info("  ✓ Scheduler fast-forwarded %d steps to match saved epoch", completed)

        start_epoch = ckpt.get("epoch", 0) + 1
        best_val_r2 = ckpt.get("val_r2", -math.inf)
        log.info("  ✓ Resuming from epoch %d  (best val R² so far: %.4f)",
                 start_epoch, best_val_r2)
    elif args.resume:
        log.warning("--resume path not found: %s  — starting from scratch.", args.resume)

    # ── Training loop ────────────────────────────────────────
    total_epochs   = start_epoch - 1 + args.epochs
    es_patience    = args.early_stop          # 0 = disabled
    es_counter     = 0
    log.info("Training epochs %d → %d  (total %d new epochs) …",
             start_epoch, total_epochs, args.epochs)
    if es_patience:
        log.info("Early stopping: patience = %d epochs", es_patience)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        # Log every 5 epochs or on first resumed epoch
        ...
        if val_res["r2"] > best_val_r2:
            ...
            if es_patience and es_counter >= es_patience:
                log.info(
                    ...
                    "no improvement for %d epochs.  Best val R²=%.4f",
                    epoch, es_patience, best_val_r2,
                )
                break
